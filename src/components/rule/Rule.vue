          ...
          >
            ...
            <el-scrollbar style="height: 470px">
              <el-form-item label="秒">
                <el-select
                    v-model="config.sec.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.sec.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
                ...
                <el-row v-if="config.sec.type===4">
                  <el-checkbox-group v-model="config.sec.checkList" @change="onExpressionChange">
              ...
              <el-form-item label="分钟" prop="min">
                <el-select
                    v-model="config.min.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.min.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
                ...
                <el-row v-if="config.min.type===4">
                  <el-checkbox-group v-model="config.min.checkList" @change="onExpressionChange">
                    <el-checkbox key="0" label="0" value="0"></el-checkbox>
                    <el-checkbox
                        v-for="cnt in 59"
                        :key="cnt"
              ...
              <el-form-item label="小时" prop="hour">
                <el-select
                    v-model="config.hour.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.hour.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
              ...
              <el-form-item label="日" prop="day">
                <el-select
                    v-model="config.day.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.day.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
              ...
              <el-form-item label="月" prop="mon">
                <el-select
                    v-model="config.mon.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.mon.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
              ...
              <el-form-item label="周" prop="week">
                <el-select
                    v-model="config.week.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.week.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
                ...
                <el-row v-if="config.week.type===2" style="color: #606266">
                  从周
                  <el-select
                      v-model="config.week.from"
                      size="small"
                      style="width:100px"
                      @change="onExpressionChange"
                  >
                    <el-option
                        v-for="item in config.week.weeks"
                        :key="item.value"
                        :label="item.name"
                        :value="item.value"
                    ></el-option>
                ...
                <el-row v-if="config.week.type===4">
                  <el-checkbox-group v-model="config.week.checkList" @change="onExpressionChange">
                    <el-checkbox
                        v-for="cnt in config.week.weeks"
                        :key="cnt.value"
                        :label="cnt.name"
                        :value="cnt.value"
                        style="width: 40px"
              ...
              <el-form-item label="年" prop="year">
                <el-select
                    v-model="config.year.type"
                    size="small"
                    style="width:250px"
                    @change="onExpressionChange"
                >
                  <el-option
                      v-for="item in config.year.types"
                      :key="item.value"
                      :label="item.name"
                      :value="item.value"
                  ></el-option>
...
export default {
  ...
  methods: {
      ...
      if (row == null) {
        ...
        this.$ruleApi.get("rule/find", {id: row.id}, (response) => {
          ...
          for (let item in this.config) {
            Object.assign(
                this.config[item],
                JSON.parse(response.data.data.var1)[item]
            );
          }
        });
      }
    ...
    onExpressionChange() {
      let expression = [];
      if (this.config.sec.type === 1) this.config.sec.value = "*";
        ...
        this.config.sec.value =
            this.config.sec.from + "/" + this.config.sec.gap;
      else this.config.sec.value = this.config.sec.checkList.join(",");

      if (this.config.min.type === 1) this.config.min.value = "*";
        ...
        this.config.min.value =
            this.config.min.from + "/" + this.config.min.gap;
      else this.config.min.value = this.config.min.checkList.join(",");

      if (this.config.hour.type === 1) this.config.hour.value = "*";
      else if (this.config.hour.type === 2)
        this.config.hour.value =
            this.config.hour.from + "-" + this.config.hour.to;
      else if (this.config.hour.type === 3)
        this.config.hour.value =
            this.config.hour.from + "/" + this.config.hour.gap;
      else this.config.hour.value = this.config.hour.checkList.join(",");

      if (this.config.day.type === 1) this.config.day.value = "*";
        ...
        this.config.day.value =
            this.config.day.from + "/" + this.config.day.gap;
      else if (this.config.day.type === 4) this.config.day.value = "L";
      else if (this.config.day.type === 5)
        this.config.day.value = this.config.day.checkList.join(",");
      else this.config.day.value = "?";

      if (this.config.mon.type === 1) this.config.mon.value = "*";
        ...
        this.config.mon.value =
            this.config.mon.from + "/" + this.config.mon.gap;
      else this.config.mon.value = this.config.mon.checkList.join(",");

      if (this.config.week.type === 1) this.config.week.value = "?";
      else if (this.config.week.type === 2)
        this.config.week.value =
            this.config.week.from + "-" + this.config.week.to;
      else if (this.config.week.type === 3)
        this.config.week.value = this.config.week.last + "L";
      else this.config.week.value = this.config.week.checkList.join(",");

      if (this.config.year.type === 1) this.config.year.value = "*";
      else
        this.config.year.value =
            this.config.year.from + "-" + this.config.year.to;
    ...
    handleNextFireQuery() {
      this.$ruleApi.get(
          "rule/nextFires",
          {cron: this.formAddOrEdit.expression},
          (response) => {
            // this.$respHandler.handleResponse(response)
